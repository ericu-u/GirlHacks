    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid():
            ...
            if User.objects.filter(username=username).exists():
                ...
                userp = UserProfile(email=email, username=username, password=password, height=height,
                                    weight=weight, age=age, excercise=excercise, diet=diet, bmi=bmi, sex=sex)
                userp.save()
                u = User.objects.get(email=email)
                user = authenticate(
                    username=u.get_username(), password=password)
                login(request, user)
